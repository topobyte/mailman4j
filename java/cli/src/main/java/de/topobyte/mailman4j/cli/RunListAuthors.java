...
import de.topobyte.mailman4j.AddressLister;
...
public class RunListAuthors
{

	final static Logger logger = LoggerFactory.getLogger(RunListAuthors.class);
			...
			throws Exception
	{
		...
		AddressLister lister = new AddressLister();
		...
		for (Mail mail : mails) {
			...
			String[] parts = mail.getAddress().split("@");
			if (parts.length != 2) {
				continue;
			}
			String account = parts[0];
			String server = parts[1];
			lister.address(account, server);
		}
